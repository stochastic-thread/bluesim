...
Sim.prototype.sendMessage = function() {
    ...
    if (c)
        if (c instanceof Array)
            for (f = c.length - 1; f >= 0; f--) e = c[f], e !== a && e.onMessage && e.onMessage.call(e, a, b);
...
Sim.Request.prototype.unlessEvent = function(a, b, c, d) {
    ...
    if (a instanceof Sim.Event) {
        ...
        for (var e = 0; e < a.length; e++) f = this._addRequest(0, b, c, d), f.msg = a[e], a[e].addWaitList(f);
    return this
};
...
Random.prototype.gamma = function(a, b) {
    if (a > 1)
        for (var c = Math.sqrt(2 * a - 1), d = a - this.LOG4, f = a + c;;) {
            ...
            for (;;)
                if (j = this.random(), g = (Math.E + a) / Math.E, j *= g, g = j <= 1 ? Math.pow(j, 1 / a) : -Math.log((g - j) / a), e = this.random(), j > 1) {
                    if (e <= Math.pow(g,
                            a - 1)) break
